...
<%@page import="com.hb.model.after.afterdao"%>
...
<%
	...
	ArrayList<afterdto> list = dao.selectAll(keyword, pStart, pEnd);
%>			
<body>
		...
		<!-- content start -->
			<div class="title">
				...
				<table class="table">
					...
					<%
						for(afterdto dto : list){
					%>
					<tr>
						<td><%=dto.getRn() %></td>
						<td><a href="afterdetail.korean?aftNo=<%=dto.getAftNo() %>"><%=dto.getAftTitle()%></a></td>
						<td><%=dto.getAftDate() %></td>
						<td><%=dto.getAftName() %></td>
					</tr>
					<%} %>
				</table>
			<p align="center">
			<%
				afterdto dto2 = (afterdto)request.getAttribute("cnt");
				total = dto2.getTotal();
