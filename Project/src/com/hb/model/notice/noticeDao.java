...
public class noticeDao {
	...
	public ArrayList<noticeDto> selectAll(String keyword, int pStart, int pEnd) {
		...
		try {
			...
			rs2 = pstmt.executeQuery("select count(*) as cnt from notice");
			if(rs2.next()){
				dto.setTotal(rs2.getInt("cnt"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}finally{
	...
	public int updateOne(int notNo, String title, String content) {
		int result = 0;
		String sql = "update notice set title="+title+",content="+content+" where notNo="+notNo;
		try {
			pstmt = conn.prepareStatement(sql);
			result = pstmt.executeUpdate();
		} catch (SQLException e) {
